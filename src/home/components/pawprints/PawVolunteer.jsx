    ...
    <div className="flex xl:flex-row flex-col w-fit xl:gap-[132px] p-5 gap-4 xl:shadow-none shadow-xl rounded-lg">
      ...
      <div className="flex flex-col xl:my-[87px] m-0 xl:w-[815px] w-full xl:gap-[32px] gap-5">
        ...
        <div className="flex flex-row justify-center xl:justify-start gap-[16px]">
          <button className="flex rounded-full xl:h-[52px] h-[42px] xl:w-[52px] w-[42px] items-center justify-center bg-[#FDF7F4] shadow-lg" onClick={() => setShow((prev) => (prev <= 0 ? length - 1 : prev - 1 ))}>
            <MdOutlineKeyboardArrowLeft className="h-[30px] w-[30px] text-[#D8C5B7]" />
          </button>
          <button  className="flex rounded-full xl:h-[52px] h-[42px] xl:w-[52px] w-[42px] items-center justify-center bg-[#8B4513] shadow-lg" onClick={() => setShow((prev) => (prev === length - 1 ? 0: prev + 1))}>
            <MdOutlineKeyboardArrowRight className="h-[30px] w-[30px] text-[#FFFFFF]" />
          </button>
        </div>
      </div>
    </div>
